...
library(dplyr)
library(glmnet)
data_for_logistic_model = data.frame(eid = biobank_residuals_data[[1]]$eid, sex = biobank_residuals_data[[1]]$sex,  value = biobank_residuals_data[[1]]$value)
for(feature in names(biobank_residuals_data)[-1]){
  feature_volume_data = data.frame(eid = biobank_residuals_data[[feature]]$eid, biobank_residuals_data[[feature]]$value)
  data_for_logistic_model = merge(data_for_logistic_model, feature_volume_data, by.x = 'eid', by.y = 'eid')
}
names(data_for_logistic_model) = c('eid','sex',names(biobank_residuals_data))
data_for_logistic_model = select(data_for_logistic_model, -'eid')
write.csv(data_for_logistic_model, 'residual_data_for_glment.csv', row.names = F)
```
...
set.seed(6431)
smp_size <- floor(0.75 * nrow(data_for_logistic_model))

train_ind <- sample(seq_len(nrow(data_for_logistic_model)), size = smp_size)
train <- data_for_logistic_model[train_ind, ]
validation_set <- data_for_logistic_model[-train_ind, ]
...
prediction_table = table(predicted_sex, validation_set$sex)
(prediction_table[1,1] + prediction_table[2,2]) / sum(prediction_table)
```
