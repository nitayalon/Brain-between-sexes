...
Added: 03/08/2020 - Dafi asked for these instead

```{r compute std for p and q alone}
mainDir = "~/Human_brain_research/bootstrap_em/results/"
p_sd = c()
q_sd = c()
for(feature_name in features_for_bootstrap){
  load(sprintf("%s/%s/%s.RData",mainDir,feature_name,feature_name))
  p_sd = c(p_sd, sd(bootstrap_results$bootstrap_results[,1]))
  q_sd = c(q_sd, sd(bootstrap_results$bootstrap_results[,2]))
}
mean(p_sd)
mean(q_sd)
```
...
```{r plotting with ggplo2}
library("RColorBrewer")
library(plot3D)
features_for_plot = c("Volume of grey matter in Crus II Cerebellum (right)", 
                      "Mean FA in cingulum cingulate gyrus on FA skeleton (right)",
                      "Mean MD in inferior cerebellar peduncle on FA skeleton (right)")
