...
This is the second analysis of the Bio Bank brain feature data. In this analysis we follow the steps decided in the meeting that took place at 31/01/2019. The analysis is composed of the following steps:
1. Regress the log brain size $X = \sum grey matter + white matter + cerebrospinal fluid$ against the log of the feature:
\[
log(feature) = \beta_0 + \beta_1X
\]
set the feature values to be the residuals
2. Normalize the residuals by replacing the original value with its  
*z-score*
3. Apply EM on the normalized data. Consider applying EM without equal variance assumption (however set constraint for the variance ratio)
