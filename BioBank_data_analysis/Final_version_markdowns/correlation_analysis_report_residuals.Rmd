...
men_cor <- cor(men_feature_data[,-c(1)], use = "pairwise.complete.obs")
women_cor <- cor(women_feature_data[,-c(1)],use = "pairwise.complete.obs")
```
...
In this subsection our goal is to establish a biological relation between the results and brain structure.

```{r Q2 features, include=FALSE}
FDR_significant_features <- rownames(combined_df_long_sorted_residuals)[
    with(combined_df_long_sorted_residuals,(fdr_pv <= sig_level & p < 0.5 & q > 0.5))]
q2_features <- intersect(data_set_columns$V2, FDR_significant_features_q2)

q2_features_names <- data_set_columns$V5[data_set_columns$V2 %in% FDR_significant_features_q2]

men_cor_q2 <- cor(select(men_feature_data,q2_features) , use = "pairwise.complete.obs")
women_cor_q2 <- cor(select(women_feature_data,q2_features),use = "pairwise.complete.obs")
```

```{r Q2 corrplot}
corrplot(men_cor_q2, order = "FPC", addrect = 10,tl.cex = 0.6,tl.srt = 45, title = "Men")
corrplot(women_cor_q2, order = "FPC", addrect = 10,tl.cex = 0.6,tl.srt = 45, title = "Women")
```


```{r Q4 features, include=FALSE}
FDR_significant_features_q4 <- rownames(combined_df_long_sorted_residuals)[
    with(combined_df_long_sorted_residuals,(fdr_pv <= sig_level & p > 0.5 & q < 0.5))]
q4_features <- intersect(data_set_columns$V2, FDR_significant_features_q4)
q4_features_names <- data_set_columns$V5[data_set_columns$V2 %in% FDR_significant_features_q4]

men_cor_q4 <- cor(select(men_feature_data,q4_features) , use = "pairwise.complete.obs")
women_cor_q4 <- cor(select(women_feature_data,q4_features),use = "pairwise.complete.obs")
```

```{r Q4 corrplot}
corrplot(men_cor_q4, order = "FPC", addrect = 10,tl.cex = 0.6,tl.srt = 45, title = "Men")
corrplot(women_cor_q4, order = "FPC", addrect = 10,tl.cex = 0.6,tl.srt = 45, title = "Women")
```


# Standardize data

```{r filter FDR significant features standardized data, include=FALSE}
sig_level <- 0.01
FDR_significant_features <- rownames(combined_df_long_sorted_standardize)[
    with(combined_df_long_sorted_standardize,(fdr_pv <= sig_level & p < 0.5 & q > 0.5 |
...
In order to determine if there's a clear pattern of masculinity among genders we compute the correlation matrix between each pair of features and then decompose the correlation matrix to its eigenvalues to look for dependent.
```{r correlation matrix by gender, include=FALSE}
men_cor <- cor(men_feature_data[,-c(1)], use = "pairwise.complete.obs")
women_cor <- cor(women_feature_data[,-c(1)],use = "pairwise.complete.obs")
```
...
```{r Q2 features, include=FALSE}
FDR_significant_features_q2 <- rownames(combined_df_long_sorted_standardize)[
    with(combined_df_long_sorted_standardize,(fdr_pv <= sig_level & p < 0.5 & q > 0.5))]
q2_features <- intersect(data_set_columns$V2, FDR_significant_features_q2)
q2_features_names <- data_set_columns$V5[data_set_columns$V2 %in% FDR_significant_features_q2]
...
```{r Q4 features, include=FALSE}
FDR_significant_features_q4 <- rownames(combined_df_long_sorted_standardize)[
    with(combined_df_long_sorted_standardize,(fdr_pv <= sig_level & p > 0.5 & q < 0.5))]
q4_features <- intersect(data_set_columns$V2, FDR_significant_features_q4)
q4_features_names <- data_set_columns$V5[data_set_columns$V2 %in% FDR_significant_features_q4]
