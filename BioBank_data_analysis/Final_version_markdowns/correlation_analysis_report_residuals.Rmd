...
pure_type_features_residuals_names <- rownames(pure_types_residual_model_residual_table)[rownames(pure_types_residual_model_residual_table) %>% 
  grep(pattern = "Volume", .)]

pure_type_features_residuals <- biobank_feature_residual_analysis[pure_type_features_residuals_names]
...
names(pure_type_men_feature_data_residuals) <- c("eid", pure_type_features_residuals_names)
...
names(pure_type_women_feature_data_residuals) <- c("eid", pure_type_features_residuals_names)
...
corrplot(pure_type_combined_correaltion_residuals, title="Residual posterior probability", order = "alphabet",
         tl.cex = 0.5,tl.srt = 45)
```

## Gender affected features

### Full list
```{r filter FDR significant features all data, include=FALSE}
sig_level <- 0.05
FDR_significant_features_residuals <- combined_df_resid_equal_probs_sorted$feature_name[
    with(combined_df_resid_equal_probs_sorted,(fdr_pv <= sig_level))]  
...
men_data_residuals <- lapply(significant_features_residuals, function(x){x$hypothesis_results$mixture_model$men_responsebilities})

women_data_residuals <- lapply(significant_features_residuals, function(x){x$hypothesis_results$mixture_model$women_responsebilities})
```
