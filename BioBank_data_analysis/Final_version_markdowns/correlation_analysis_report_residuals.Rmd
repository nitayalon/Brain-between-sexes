...
pure_types_features_names_by_type <- pure_types_features_names[
  c(pure_types_features_names %>% grep("Volume",.),
          pure_types_features_names %>% grep("MD",.))]

pure_type_features_residuals <- biobank_feature_residual_analysis[pure_types_features_names_by_type]
...
LabelCol = c("black", rep("blue",5), "black")
corrplot(pure_type_combined_correaltion_residuals, order = "alphabet", method = 'number',
         tl.cex = 0.5,tl.srt = 45, tl.col=LabelCol,
         col= colorRampPalette(c("red","green","blue"))(10) ,
         cl.lim = c(-1,1))
```

## Single mixture model (Equal probabilities)
...
corrplot(combined_correaltion_residuals[Volume_features_residuals,Volume_features_residuals], order = "alphabet",
         tl.cex = 0.1,tl.srt = 45)
```

```{r corrplot of MD features single mixture model}
corrplot(combined_correaltion_residuals[MD_features_residuals,MD_features_residuals], order = "alphabet",
         tl.cex = 0.1,tl.srt = 45)
```
```{r corrplot of FA features single mixture model}
corrplot(combined_correaltion_residuals[FA_features_residuals,FA_features_residuals], order = "alphabet",
         tl.cex = 0.1,tl.srt = 45)
```
