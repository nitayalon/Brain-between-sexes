...
biobank_feature_residual_analysis <- biobank_full_analysis_list
...
sapply(biobank_feature_residual_analysis, function(x){x$hypothesis_results$cohen_d_test$estimate}) %>% 
  data.frame()
...
combined_df_long_sorted_residuals %>% 
  filter(bins_for_fdr %in% levels(combined_df_long_sorted_residuals$bins_for_fdr)[c(1,2)]) %>% 
...
combined_df_resid_equal_probs_sorted %>% 
  ...
  inner_join(non_pure_types_features_data_set,
             by = 'feature_name')
...
combined_df_resid_equal_probs_sorted %>% 
  ...
  coord_fixed()
```
```{r exporting q2 q4 features}
q2_q4_features_names <- 
  combined_df_resid_equal_probs_sorted %>% 
  filter(!p_equal_q) %>% 
