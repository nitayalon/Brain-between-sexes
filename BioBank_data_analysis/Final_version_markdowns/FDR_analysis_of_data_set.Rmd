...
combined_df_resid_equal_probs <- combined_df_resid_equal_probs[,c(2,3,4)]
names(combined_df_resid_equal_probs) <- c("p_value", "p", "q")
rownames(combined_df_resid_equal_probs) <- rownames(pvalue_mixture_vs_equal_prob_pv)
...
combined_df_resid_equal_probs <- base::merge(combined_df_resid_equal_probs, cohens_d_per_feature_residuals_equal_probs,by = 0, all = T)

names(combined_df_resid_equal_probs) <- c("feature_name", "p_values","p","q","Cohen_D")
```

```{r pure type vs mixture model plotting with colors}
combined_df_resid_equal_probs$bins_of_pv <- cut(combined_df_resid_equal_probs$p_values, c(1e-9,1e-8,1e-3,1e-2,1))
```

```{r fdr procedure, include = FALSE}
N <- nrow(combined_df_resid_equal_probs)
combined_df_resid_equal_probs_sorted <- combined_df_resid_equal_probs[order(combined_df_resid_equal_probs$p_values,decreasing = F),]
rownames(combined_df_resid_equal_probs_sorted) = rownames(combined_df_resid_equal_probs_sorted)[order(combined_df_resid_equal_probs$p_values,decreasing = F)]
combined_df_resid_equal_probs_sorted$i <- seq(1:N)
combined_df_resid_equal_probs_sorted$bh_pvalue <- with(combined_df_resid_equal_probs_sorted, p_values * i / N)
...
ggplot(combined_df_resid_equal_probs_sorted[combined_df_resid_equal_probs_sorted$bins_for_fdr %in% levels(combined_df_resid_equal_probs_sorted$bins_for_fdr)[c(1,2)],], aes(p,q, colour = Cohen_D)) +
  geom_point(aes(shape = bins_for_fdr)) +
  scale_shape_manual(values=c(15, 17))+
  geom_vline(xintercept = 0.5) +
  geom_hline(yintercept = 0.5) +
...
```{r selecting extreme gender oriented features, include=FALSE}
top_20_gender_features <- combined_df_resid_equal_probs_sorted$feature_name[order(combined_df_resid_equal_probs_sorted$fdr_pv, decreasing = T)][1:20]
```
