...
load("~/Human_brain_research/Data/Biobank/biobank_feature_residual_analysis.RData")
load("~/Human_brain_research/Data/Biobank/BioBank_residual_data.RData")
```
...
```{r combine the EM results with the FDR results for export}
write.csv(pure_types_residual_model_table, file = "~/Human_brain_research/data_for_paper/Tables/pure_types_features_residual_data.csv", row.names = T)
```
...
combined_df_resid_equal_probs_sorted$feature_name <- as.character(combined_df_resid_equal_probs_sorted$feature)
...
Last (April 2020), we check the nubmer ofg features men and women sample diffrentley

```{r checking number of significantly different features}
mainly_women <- mosaic_mixture_model_features %>%
  filter(p < 0.5 & q > 0.5)

mainly_men <- mosaic_mixture_model_features %>%
  filter(p > 0.5 & q < 0.5)

nrow(mainly_women) + nrow(mainly_men)

q2_q4_features_names <- c(mainly_women$feature, mainly_men$feature)

min(mainly_men$p);max(mainly_men$p)
min(mainly_men$q);max(mainly_men$q)
which.max(mainly_men$p)
which.min(mainly_men$p)
mainly_men[7,]$q
```
