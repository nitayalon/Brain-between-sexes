...
p_and_q <- sapply(feature_residual_m_parameters, function(x){c(x$p, x$q)})
```
...
combined_df_long_sorted_residuals <- combined_df_long[order(combined_df_long$p_values,decreasing = F),]
...
combined_df_long_sorted_residuals$bins_for_fdr <- cut(combined_df_long_sorted_residuals$fdr_pv, c(0,1e-2,5e-2,1))
```
...
ggplot(combined_df_long_sorted_residuals[combined_df_long_sorted_residuals$bins_of_pv %in% levels(combined_df_long_sorted_residuals$bins_of_pv)[c(1,2,3)],], aes(p,q, colour = Cohen_D)) + 
...
random_feature_main <- sample(rownames(combined_df_long_sorted_residuals)[
    with(combined_df_long_sorted_residuals,(fdr_pv <= sig_level & region == "Main"))],1)  


random_feature <- sample(FDR_significant_features,1)
significant_features[[random_feature]]$hypothesis_results$t_test_for_difference_between_genders
significant_features[[random_feature]]$hypothesis_results$pure_type_vs_mixed_gender_em_results$alternative_hypothesis$m_parameters

significant_features[[random_feature_main]]$hypothesis_results$t_test_for_difference_between_genders
significant_features[[random_feature_main]]$hypothesis_results$pure_type_vs_mixed_gender_em_results$alternative_hypothesis$m_parameters
```
...
```{r fdr p value by quadrant, warnings = FALSE}
ggplot(combined_df_long_sorted[combined_df_long_sorted$bins_of_pv %in% levels(combined_df_long_sorted$bins_of_pv)[c(1,2)],], aes(p,q, colour = Cohen_D)) + 
  geom_point(aes(shape = bins_for_fdr)) +
  scale_shape_manual(values=c(3, 6, 17))+
  geom_vline(xintercept = 0.5) +
  geom_hline(yintercept = 0.5) +
