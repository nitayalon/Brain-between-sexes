...
brain_volume <- columns_names$UDI
total.brain.volum.data <- bio_bank_data %>% select("X25010.2.0","X25004.2.0")
user_data <- select(bio_bank_data, eid, X31.0.0)
# list.of.names <- names(full_relevant_data)[-c(1:5)]
list.of.names <- names(relevant_data)
```

```{r editing the columns names dictionary}
data_set_columns$V2 <- gsub("-", ".", paste0("X",data_set_columns$V2))
```

```{r quick QA}
length(intersect(data_set_columns$V2, list.of.names))
```
...
biobank_residuals_data <- pblapply(list.of.names,function(x){
  feature_residuals <- methodHelperOfResidualComputation(x,
                                                        user_data)
})
...
biobank_normalized_data <- list()
biobank_normalized_data <- pblapply(list.of.names,function(x){
  feature_residuals <- methodHelperFeatureNormalization(x,
                                                        user_data)
})
names(biobank_normalized_data) <- list.of.names
save(biobank_normalized_data, file = "~/Human_brain_research/BioBank_data_analysis/Final_data_for_analysis/BioBank_standardized_data.RData")
```

Next edit the code for EM computation and apply it on both data sets

```{r testing the function}
applyHypothesisOverResiduals(biobank_normalized_data$X25056.2.0)
```
...
biobank_feature_standard_analysis <- pblapply(biobank_normalized_data,function(x){
  BrainFeatureAnalysis(x)
})
