...
full_relevant_data %>% 
  ...
  group_by(Sex) %>% 
  summarise(avg = mean(age), std = sd(age), count = n())
```
...
Evaluate the number of loss information:
```{r number of loss data points}
countNumberOfMissingData = function(feature_name,
                                           subject_data){
  ...
  return(nrow(raw_feature_data) - nrow(logged_feature_data))
}

removed_data = pblapply(list.of.names,function(x){
countNumberOfMissingData(x,user_data)
})

hist(unlist(removed_data))
```
...
save(biobank_standardized_data, file = "~/Human_brain_research/Data/Biobank/BioBank_standardized_data.RData")
```
...
biobank_feature_residual_analysis <- pblapply(biobank_residuals_data,function(x){
  BrainFeatureAnalysis(x,including_equal_mixture_model = T)
})
