...
men_density <- logConDens(men_resid, xgrid = grid ,smoothed = T)
women_density <- logConDens(women_resid, xgrid = grid ,smoothed = T)

pop_llk <- computeModelBasedLogLikeihoodRatio(em_params, men_resid, women_resid)
plotGenderHistogram(results.list[[k]], lower_antidiagonal_feature_names[k])
plot(pop_llk$population, pop_llk$llrt)
abline(h = 0)
...
pop_llk <- computeModelBasedLogLikeihoodRatio(em_params, men_resid, women_resid)
plotGenderHistogram(results.list[[j]], upper_antidiagonal_feature_names[j])
plot(pop_llk$population, pop_llk$llrt)
abline(h = 0)
plot(pop_llk$population, pop_llk$men_likelihood, 
     cex = 0.5, col = "blue", ylim = c(0,0.5))
...
quantile(c(men_resid,women_resid), c(0.025,0.975))

trim_value <- 3
ind <- non_parametric_llrt$men_density$xs > -trim_value & non_parametric_llrt$men_density$xs < trim_value
plot(non_parametric_llrt$men_density$xs[non_parametric_llrt$men_density$xs > -trim_value & non_parametric_llrt$men_density$xs < trim_value], log(non_parametric_llrt$men_density$f.smoothed[ind] / 
           non_parametric_llrt$women_density$f.smoothed[ind]))
```