...
combined_df_long_sorted$bins_for_fdr <- cut(combined_df_long_sorted$fdr_p_value, c(0,1e-3,1e-1,1000))
...
upper_main_diagonal_feature_names <- 
  rownames(combined_df_long_sorted)[
    with(combined_df_long_sorted,(p_values < 1e-5 & p > 0.5 & q > 0.5))]  
...
upper_antidiagonal_feature_names <- 
  rownames(combined_df_long_sorted)[
    with(combined_df_long_sorted,(p_values < 1e-6 & p < 0.5 & q > 0.5))]  
...
plotGenderHistogram(lower_antidiagonal_features[["X25872.2.0"]], "X25872.2.0")
```
