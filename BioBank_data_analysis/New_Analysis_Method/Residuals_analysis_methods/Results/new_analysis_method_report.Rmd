...
source("~/HumanBrainResearch/BioBank_data_analysis/New_Analysis_Method/Residuals_analysis_methods/analyze_full_brain_feature.R")
source("~/HumanBrainResearch/BioBank_data_analysis/New_Analysis_Method/Residuals_analysis_methods/method_helper.R")
source("~/HumanBrainResearch/BioBank_data_analysis/load_biobank_data.R")

list.of.names <- names(results.list)
```
...
Plotting the p-value for the first hypothesis:
```{r p value of single distribution vs composite distribution}
simple_vs_mixture_model_pvalue <- sapply(results.list, function(x){log(max(1 - pchisq(x$hypothesis_results$simple_vs_compostie_llrt, 4),1e-8))})

hist(simple_vs_mixture_model_pvalue, 1e-8, breaks = 100)
```
...
significat_level = log(0.001)
(simple_vs_mixture_model_pvalue < significat_level) %>% 
  table() %>% 
  prop.table()
```
...
list_of_single_distribution_features <- list.of.names[simple_vs_mixture_model_pvalue > significat_level]

analyzeFullBrainFeature(results.list[names(results.list) == list_of_single_distribution_features[1]],plot = T)

analyzeFullBrainFeature(results.list[names(results.list) == list_of_single_distribution_features[2]],plot = T)
```

In contrary, if we take for example a high llr feature:
```{r plotting significant features to validate the llr}
list_of_composite_distribution_features <- list.of.names[simple_vs_mixture_model_pvalue < significat_level]
...
pure_vs_mixture_model_pvalue <- sapply(pure_vs_mixture_candidates, function(x){log(max(1 - pchisq(-2*x$hypothesis_results$pure_vs_mixed_llrt,2), 1e-8))})
```
...
hist(pure_vs_mixture_model_pvalue, breaks = 100, main = "Histogram of log(pv)",sub = "Pure type vs Composite", xlab = "log(pv)")

(pure_vs_mixture_model_pvalue < log(0.001)) %>% table() %>% prop.table()
```
...
list_of_pure_type_distribution_features <- list_of_names_pure_vs_mixture[pure_vs_mixture_model_pvalue > significat_level]
...
list_of_mixture_type_distribution_features <- list_of_names_pure_vs_mixture[pure_vs_mixture_model_pvalue < significat_level]
...
```{r features with weak gender preferance}
...
gender_preference_features_names <- sapply(mixture_features, 
                  function(x){x$hypothesis_results$pure_type_vs_mixed_gender_em_results$alternative_hypothesis$m_parameters$p > p_level &&                       x$hypothesis_results$pure_type_vs_mixed_gender_em_results$alternative_hypothesis$m_parameters$q < q_level})
...
analyzeFullBrainFeature(gender_preference_features[2],plot = T)
```
...
# Summary
The new analysis method yields very intersting results. First is seems that more features fits the single distribution model than before. Second, a visual inspection of the results shows that the new analysis method separates between pure types and mixture models with greater power.  
